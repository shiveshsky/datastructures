    ...
    def wordBreak(self, s, wordDict):
        ...
        if len(s) == 1:
            if s in wordDict:
                ...
                self.notfounddp.add(s)
                return False
        if s in wordDict:
            return True
        if s in self.notfounddp:
            return False
        for i in range(1, len(s)):
            res = self.wordBreak(s[i:], wordDict)
            if s[0:i] in wordDict and res:
                return True
            else:
                if not res:
                    self.notfounddp.add(s[i:])
                if not s[0:i] in wordDict:
                    self.notfounddp.add(s[0:i])
        return False


if __name__ == '__main__':
    # print(Solution().wordBreak("leetcode", ["leet", "code", "d", "e"]))
    print(Solution().wordBreak("fohhemkkaecojceoaejkkoedkofhmohkcjmkggcmnami",
                               ["kfomka", "hecagbngambii", "anobmnikj", "c", "nnkmfelneemfgcl", "ah", "bgomgohl",
                                "lcbjbg", "ebjfoiddndih", "hjknoamjbfhckb", "eioldlijmmla", "nbekmcnakif",
